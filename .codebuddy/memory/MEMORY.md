...
## 项目事实（稳定，截至 v1.9.36 / 2026-09-23，经实地核查刷新）
...
- **版本单一来源 = `src/version.js` 的 `VERSION`**（当前 1.9.36）。全局展示位须同步：
...
- **测试**：Node 内置 `node --test`（零依赖，v1.9.30 起替代未启用的 Jest）。`tests/` 共 4 文件 / 12 用例
  （collect-codes 2、collect-dict 3、smoke 3、**url-guard 4**）。**仍无 API 路由 / SSE 集成测试**（T5 待办）。
- **质量现状**（v1.9.36 核查）：`npm run lint` 0 error/0 warning；`tsc --noEmit` 通过（strict:true）；
  `src/` 119 文件 / 9114 行；**全部代码文件 ≤200 行**（0 超出，但 configUI.js 191 / virtualDom/manager.js 190 /
  useCollector.ts 189 / performanceMonitor.js 186 贴线，需防回潮）。
...
- **剩余待办（见 `docs/TASKS.md`）**：T5 API 路由集成测试、T6 近 200 行门禁、T7 依赖审计 CI、
  T8 a11y 走查、T9 命名澄清、T10 词典趋势可视化。
