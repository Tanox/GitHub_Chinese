...
## 项目事实（GitHub_Chinese，截至 2026-09-25，v1.11.15）
...
- **版本单一来源 = `src/version.js` 的 `VERSION`**（当前 **1.11.15**，工作树未提交；HEAD 1.11.14）。全局展示位同步：`package.json`/README 徽章/CHANGELOG 小节/被改文件头注释。**每次改动 bump 最小版本（项目惯例按任务逐 patch 递增，如 T16→1.11.8…T36→1.11.15）；仅更新被改文件头注释，禁止全仓库批量刷写。**
